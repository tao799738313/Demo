...
var tt_util = {
    ...
    getLink:function(url) {
        ...
        if(search.length==2){
            ...
            linkArr.forEach(function(arr,index){
                if(arr[1]){
                    link = link + arr[0] + "=" + (arr[1] || "")
                    if(len != index+1){
                        link = link + "&"
                    }
                }
            })
            var link = origin + pathname + link;
            return link
        }else{
            var link = origin + pathname
            return link
        }
    },
    isWX: function(cb) {
                    ...
                    t(h.onMenuShareTimeline, {
                        complete: function() {
                            i("shareTimeline", {
                                ...
                                img_url: e.imgUrl || "",
                                // 这里
                                link: e.link || link || location.href,
                                type: e.type || "link",
                                data_url: e.dataUrl || ""
                            }, e)
                        }
