...
function init(){
	$('#loadMask').show()
	$.ajax({
		url:'http://localhost:8080/rp_project_hunan/app/hnGsData1224/getGs',
		data:{
...
function draw(data,id,dom){
	...
	drawDWTZ(data.dwtzsList,id,'dwtzList',dom)
...
function drawFZJG(arr,id,arrName,dom){
    ...
    arr.forEach(function(opt){
    	if(opt.isXz == '1'){
			...
			str += `<div><span class="gs-${arrName}-no" >${opt.name}</span></div>`
		}
    })
	// 二次预防
	if(str){
		...
		$(dom).children().eq(len-1).remove()
		$(dom).append(`<div class="mgLB">
		<span class="noArr">${nameObj[arrName]}</span>
		</div>`)
	}
}
